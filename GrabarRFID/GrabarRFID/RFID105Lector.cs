...
namespace Sitacomm.RFID
{
    ...
    {   
        ...
        public int EscribirEPC(string EPC, string PWD)
        {
            ...
            if (PWD.Length < 8)
            {
                return 1;//mala contraseña
            }

            if ((EPC.Length % 4) != 0)
            {
                return 2; //longitud de epc no válido
            }
            WriteEPClen = Convert.ToByte(EPC.Length / 2);
            ENum = Convert.ToByte(EPC.Length / 4);
            byte[] EPCb = new byte[ENum];
            EPCb = HexStringToByteArray(EPC);
            byte[] fPassWord = HexStringToByteArray(PWD);
            int ferrorcode = 0;
            int fCmdRet = StaticClassReaderB.WriteEPC_G2(ref m_direccionCom, fPassWord, EPCb, WriteEPClen, ref ferrorcode, m_indicePuerto);
        ...
        {
            int resultado = 0;
            //  resultado = StaticClassReaderB.OpenComPort(11, ref m_direccionCom, m_velocidad, ref m_indicePuerto);
          //  m_puerto = 11;
